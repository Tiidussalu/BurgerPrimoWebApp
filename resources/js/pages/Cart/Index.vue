<template>
  ...
  <div class="page-root text-white">
          ...
          <div class="relative glass rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            ...
            <div class="p-6">
              ...
              <div class="flex gap-3 mt-6">
                <button @click="modal.show = false" class="flex-1 py-3 rounded-xl border border-white/10 text-gray-300 hover:bg-white/5 transition-colors font-semibold text-sm">Tühista</button>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 py-10 pb-24">
      <div class="mb-8">
        <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-white/10 bg-white/5 text-gray-400 text-xs font-semibold uppercase tracking-widest mb-4">Ostukorv</div>
        <h1 class="text-4xl font-bold mb-2">Teie ostukorv</h1>
      ...
      <div v-if="cartItems.length > 0" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div v-for="item in cartItems" :key="item.type === 'custom_burger' ? `burger-${item.id}` : `menu-${item.id}`" class="glass p-6 hover:border-[#D2691E]/25 transition-colors duration-200">
            ...
            <template v-if="item.type === 'custom_burger'">
              <div class="flex gap-4">
                <div class="flex-1">
                  <span class="inline-block text-xs bg-yellow-500/15 text-yellow-400 px-2.5 py-1 rounded-full font-bold mb-3">Kohandatud burger</span>
                ...
                <div class="flex flex-col items-end justify-between">
                  <button @click="confirmRemoveItem(item.id)" class="text-red-400 hover:text-red-300 p-2 hover:bg-red-500/10 rounded-lg transition-colors" title="Eemalda">
            ...
            <template v-else-if="item.type === 'menu_item'">
              <div class="flex gap-4">
                <div class="flex-1">
                  <span class="inline-block text-xs bg-[#D2691E]/15 text-[#D2691E] px-2.5 py-1 rounded-full font-bold mb-3">Menüü</span>
                  ...
                  <div class="text-sm text-gray-500 space-y-1">
                    ...
                    <div v-if="item.needs_utensils" class="flex items-center gap-1.5 text-gray-400"><span class="w-1 h-1 rounded-full bg-gray-600 shrink-0" />Söögiriistad kaasas</div>
        ...
        <div class="lg:col-span-1">
          <div class="glass p-6 sticky top-24">
            <h2 class="text-xl font-bold mb-6">Kokkuvõte</h2>
            ...
            <div class="border-t border-white/6 pt-4 mb-6">
              <div class="flex justify-between items-baseline">
                <span class="font-bold">Kokku</span>
      ...
      <div v-else class="text-center py-24">
        ...
        <h2 class="text-3xl font-bold mb-3">Ostukorv on tühi</h2>
        <p class="text-gray-400 mb-8">Lisa tooteid menüüst või ehita oma burger</p>
        <div class="flex gap-4 justify-center flex-wrap">
          <Link href="/menu" class="btn-magnetic inline-flex items-center gap-2 px-8 py-4 bg-[#D2691E] hover:bg-[#B8571A] text-white rounded-2xl font-bold text-sm transition-all">Vaata menüüd</Link>
          <Link href="/burger-builder" class="btn-magnetic inline-flex items-center gap-2 px-8 py-4 glass border-[#D2691E]/40 text-white rounded-2xl font-bold text-sm transition-all hover:border-[#D2691E]">Ehita burger</Link>
        </div>
      </div>
    </main>
...
const getSizeName = (size?: string) => ({ small: 'Väike', medium: 'Keskmine', large: 'Suur' }[size ?? ''] ?? size ?? '')
...
const confirmClearCart = () => {
  openModal({
    ...
    message: 'Kas olete kindel, et soovite kogu korvi tühjendada?',
