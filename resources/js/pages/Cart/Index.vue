          ...
          <div class="relative glass rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            ...
            <div class="p-6">
              ...
              <div class="flex gap-3 mt-6">
                <button @click="modal.show = false" class="flex-1 py-3 rounded-xl border border-white/10 text-gray-300 hover:bg-white/5 transition-colors font-semibold text-sm cursor-pointer">{{ t('cart.modal.cancel') }}</button>
                <button @click="modal.onConfirm(); modal.show = false" :class="modal.type === 'danger' ? 'bg-red-600 hover:bg-red-500' : 'bg-[#D2691E] hover:bg-[#B8571A]'" class="flex-1 py-3 rounded-xl font-bold text-sm text-white transition-all shadow-lg cursor-pointer">{{ modal.confirmLabel }}</button>
              </div>
            </div>
          </div>
        </div>
            ...
            <template v-if="item.type === 'custom_burger'">
              <div class="flex gap-4">
                ...
                <div class="flex flex-col items-end justify-between">
                  <button @click="confirmRemoveItem(item.id)" class="text-red-400 hover:text-red-300 p-2 hover:bg-red-500/10 rounded-lg transition-colors cursor-pointer" :title="t('cart.remove')">
          ...
          <div class="glass p-6 sticky top-24">
            ...
            <div class="mb-6">
              ...
              <div class="grid grid-cols-3 gap-3">
                <button type="button" @click="deliveryMethod = 'dine_in'" :class="deliveryMethod === 'dine_in' ? 'border-[#D2691E] bg-[#D2691E]/10 text-white' : 'border-white/8 text-gray-500 hover:border-white/20 hover:text-gray-300'" class="py-4 rounded-xl border-2 flex flex-col items-center gap-2 transition-all cursor-pointer">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                  <span class="text-xs font-semibold">{{ t('cart.dine_in') }}</span>
                </button>
                <button type="button" @click="deliveryMethod = 'takeaway'" :class="deliveryMethod === 'takeaway' ? 'border-[#D2691E] bg-[#D2691E]/10 text-white' : 'border-white/8 text-gray-500 hover:border-white/20 hover:text-gray-300'" class="py-4 rounded-xl border-2 flex flex-col items-center gap-2 transition-all cursor-pointer">
            ...
            <button @click="checkout" class="btn-magnetic w-full py-4 rounded-xl bg-[#D2691E] hover:bg-[#B8571A] text-white font-bold text-sm transition-all shadow-lg shadow-[#D2691E]/20 cursor-pointer">
              {{ t('cart.checkout') }}
            </button>
            <button @click="confirmClearCart" class="w-full mt-3 text-red-400 hover:text-red-300 py-2.5 rounded-xl hover:bg-red-500/8 transition-colors text-sm font-medium cursor-pointer">
              {{ t('cart.clear') }}
            </button>
          </div>
        </div>
