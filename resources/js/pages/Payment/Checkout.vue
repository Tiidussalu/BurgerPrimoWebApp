<template>
  <div class="min-h-screen bg-[#0B0B0B] text-white">
    ...
    <!-- Main Content -->
    <main class="max-w-4xl mx-auto px-4 sm:px-6 py-8 pt-24 lg:pt-28">
      <!-- Header -->
      <div class="mb-8">
...
const mountCardElement = () => {
  ...
  const elements = stripe.elements({ locale: locale.value });
...
// Remount Stripe card element when locale changes so placeholder text updates
watch(locale, () => {
  mountCardElement();
});
...
const handleSubmit = async () => {
  ...
  if (!stripe || !cardElement) {
    cardError.value = t('checkout.err.stripe');
    return;
  }
