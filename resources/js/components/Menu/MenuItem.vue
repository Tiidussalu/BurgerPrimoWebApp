<template>
  ...
  <div
    ...
    :class="highlighted ? 'ring-2 ring-[#D2691E] bg-[#161616]' : ''"
  >
    ...
    <div class="relative flex-shrink-0 w-28">
      ...
      <button @click.stop="toggleFavorite" class="absolute top-2 left-2 w-7 h-7 bg-black/60 hover:bg-black/80 rounded-lg flex items-center justify-center transition-all z-10">
      ...
      <button @click.stop="openAddonModal" class="absolute bottom-2 right-2 w-8 h-8 bg-[#D2691E] hover:bg-[#E07A2E] text-white rounded-lg flex items-center justify-center shadow-lg transition-all duration-200 opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0">
  ...
  <!-- ── Vertical card ── -->
  <div v-else class="bg-[#121212] rounded-2xl overflow-hidden hover:ring-2 hover:ring-[#D2691E] transition-all duration-300 group">
