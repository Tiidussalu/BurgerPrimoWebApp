    ...
    <EditableSection section-id="reviews" @save="save" @cancel="cancel">
      <template #default="{ isEditing }">
        ...
        <div class="md:hidden relative">
          ...
          <button v-if="reviews.length > 1" @click="prevReviewSlide" class="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 bg-black/60 text-white w-9 h-9 rounded-full flex items-center justify-center text-2xl hover:bg-black/80 transition z-10">‹</button>
          <button v-if="reviews.length > 1" @click="nextReviewSlide" class="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 bg-black/60 text-white w-9 h-9 rounded-full flex items-center justify-center text-2xl hover:bg-black/80 transition z-10">›</button>
