...
const subjects = computed(() => [
  { value: 'reservation', label: t('contact.sub.reservation') },
  { value: 'catering',    label: t('contact.sub.catering')    },
  { value: 'feedback',    label: t('contact.sub.feedback')    },
  { value: 'other',       label: t('contact.sub.other')       },
])

const hours = computed(() => [
  { days: t('contact.hours.mon'), time: '11:00 – 22:00' },
  { days: t('contact.hours.fri'), time: '11:00 – 23:00' },
  { days: t('contact.hours.sun'), time: '12:00 – 21:00' },
])
      ...
      <div class="glass px-6 py-12 md:px-12 md:py-14">
        ...
        <div class="text-center mb-12 space-y-4">
          ...
          <h2 :ref="(el) => headingRef = el as any" class="text-3xl md:text-5xl font-bold text-white">
            {{ t('contact.heading') }} <span class="text-[#D2691E]">{{ t('contact.heading.accent') }}</span>
          </h2>
          ...
          <div :ref="(el) => leftRef = el as any" class="space-y-4">
            ...
            <div class="glass-card p-5 group hover:border-[#D2691E]/22 transition-colors duration-300 relative overflow-hidden">
              ...
              <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="text-sm font-bold text-[#F5DEB3]">{{ t('contact.location.title') }}</h3>
              </div>
            ...
            <div class="glass-card p-5 group hover:border-[#D2691E]/22 transition-colors duration-300 relative overflow-hidden">
              ...
              <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="text-sm font-bold text-[#F5DEB3]">{{ t('contact.hours.title') }}</h3>
              </div>
            ...
            <div class="glass-card overflow-hidden relative">
              ...
              <div class="p-5 lg:p-7">
                <div class="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h3 class="text-sm font-bold text-[#F5DEB3]">{{ t('contact.form.title') }}</h3>
                    <p class="text-xs text-gray-600 mt-0.5">{{ t('contact.form.reply') }}</p>
                  </div>
                </div>
                ...
                <form v-if="submitState !== 'success'" @submit.prevent="submitContact" class="space-y-4">
                  <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 -translate-y-2" enter-to-class="opacity-100 translate-y-0">
                    <div v-if="submitState === 'error'" class="px-4 py-3 bg-red-500/10 border border-red-500/20 rounded-xl"><p class="text-red-400 text-sm">{{ t('contact.form.error') }}</p></div>
                  </Transition>

                  <div>
                    <label class="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wider">{{ t('contact.form.name') }} <span class="text-[#D2691E]">*</span></label>
                    <div class="relative">
                      <input v-model="form.name" type="text" required :placeholder="t('contact.form.name.ph')" @focus="focusedField = 'name'" @blur="focusedField = null" class="primo-input w-full px-4 py-3 text-sm bg-black/40 border border-white/10 rounded-xl text-white placeholder-gray-700" />
                  ...
                  <div>
                    <label class="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wider">{{ t('contact.form.email') }} <span class="text-[#D2691E]">*</span></label>
                    <div class="relative">
                  ...
                  <div>
                    <label class="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wider">{{ t('contact.form.message') }} <span class="text-[#D2691E]">*</span></label>
